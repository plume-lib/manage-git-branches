...
## Checks that `remote_is_usable REMOTES NAME` answers EXPECTED, which is
## "yes" or "no".
check_usable() {
  if remote_is_usable "$1" "$2"; then
...
# A URL or a pathname, which git accepts wherever it accepts a remote's name.
# Git rejects any string that contains "/", or that is "." or "..", as the name
# of a remote, so such a string can only be a URL or a pathname:  it is not the
# name of a remote that this clone is missing.  `git push --set-upstream
# ../other.git BRANCH` writes such a value into `branch.BRANCH.remote`.
check_usable "${REMOTES}" 'mirrors/other.git' 'yes'
check_usable "${REMOTES}" '../other.git' 'yes'
check_usable "${REMOTES}" '/srv/git/other.git' 'yes'
