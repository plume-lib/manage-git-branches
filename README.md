...
* [`git-new-branch`](git-new-branch) `BRANCHNAME`:
  ...
  new branch, so the new branch has no upstream branch.  `git-push-to` and
  `git-pull-from` need one; to give the new branch an upstream branch, cd to
  the new directory and run `git push --set-upstream origin BRANCHNAME`.
